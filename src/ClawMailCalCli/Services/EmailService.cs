	...
	: IEmailService
{
	...
	public async Task<IReadOnlyList<EmailSummary>> GetEmailsAsync(string? folderName = null, CancellationToken cancellationToken = default)
	{
		...
		{
			logger.LogDebug("Listing emails for account '{AccountName}', folder '{FolderName}'.", defaultAccount.Name, folderName ?? "inbox");
		}
		...
		try
		{
			...
			return await graphClientService.GetFolderMessagesAsync(defaultAccount.Name, folderName, DefaultMessageCount, cancellationToken);
		}
		...
		{
			AnsiConsole.MarkupLine($"[red]Error:[/] Folder '[yellow]{Markup.Escape(folderName!)}[/]' was not found.");

			if (logger.IsEnabled(LogLevel.Debug))
			{
				logger.LogDebug(invalidOperationException, "Folder '{FolderName}' was not found for account '{AccountName}'.", folderName, defaultAccount.Name);
			}
