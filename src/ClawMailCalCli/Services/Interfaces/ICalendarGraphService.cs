...
public interface ICalendarGraphService
{
	...
	/// <summary>
	/// Deletes a calendar event by its unique Graph event ID.
	/// </summary>
	/// <param name="accountName">The account name whose credentials are used for the request.</param>
	/// <param name="eventId">The unique Microsoft Graph event identifier.</param>
	/// <param name="cancellationToken">A token to cancel the asynchronous operation.</param>
	/// <returns><see langword="true"/> if the event was deleted successfully; otherwise <see langword="false"/>.</returns>
	Task<bool> DeleteEventByIdAsync(string accountName, string eventId, CancellationToken cancellationToken = default);
}
